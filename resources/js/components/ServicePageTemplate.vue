<template>
  <div class="space-y-12">
      ...
      <div class="max-w-5xl">
        ...
        <div class="mt-8 flex flex-wrap gap-3">
          <router-link :to="page.primaryLink || '/contacts'" class="btn-primary">
            {{ page.primaryLabel || 'Оставить заявку' }}
          </router-link>
          <router-link :to="page.secondaryLink || '/pricing'" class="btn-secondary">
            {{ page.secondaryLabel || 'Форматы работы' }}
          </router-link>
        </div>
      </div>
    </section>
    ...
    <section class="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
      <div>
        <div class="text-xs uppercase tracking-[0.18em] text-slate-500">Что входит</div>
    ...
    <section class="space-y-6">
      <div class="max-w-3xl">
        <div class="text-xs uppercase tracking-[0.18em] text-slate-500">Как идёт работа</div>
        <h2 class="mt-3 text-3xl font-semibold tracking-tight text-white md:text-5xl">
          {{ page.stagesTitle }}
        </h2>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <article
          v-for="(item, index) in page.stages"
          :key="item.title"
          class="rounded-[28px] border border-white/10 bg-white/[0.04] p-6"
        >
          <div class="text-sm font-semibold text-cyan-200">0{{ index + 1 }}</div>
    ...
    <section
      v-if="page.legal"
      class="rounded-[32px] border border-amber-400/20 bg-[linear-gradient(135deg,rgba(245,158,11,0.12),rgba(255,255,255,0.04))] p-8 md:p-10"
    >
      <div class="max-w-4xl">
        <div class="text-xs uppercase tracking-[0.18em] text-amber-200/80">Правовой контур</div>
        <h2 class="mt-3 text-3xl font-semibold tracking-tight text-white md:text-5xl">
          {{ page.legal.title }}
        </h2>
        <p class="mt-4 text-base leading-8 text-slate-200 md:text-lg">
          {{ page.legal.text }}
        </p>

        <div class="mt-6 grid gap-4 md:grid-cols-2">
          <div
            v-for="item in page.legal.items"
            :key="item.title"
            class="rounded-3xl border border-white/10 bg-white/[0.05] p-5"
          >
            <div class="text-base font-semibold text-white">{{ item.title }}</div>
            <div class="mt-2 text-sm leading-7 text-slate-300">{{ item.text }}</div>
          </div>
        </div>
      </div>
    </section>
