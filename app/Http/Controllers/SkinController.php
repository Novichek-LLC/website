...
class SkinController extends Controller
{
    ...
    public function skin($uuid, $side = false, $size = 224){
        ...
        try {
            ...
            $image_skin = Image::read($storage->get($filename));
        } catch (FileNotFoundException $e) {
            $image_skin = Image::read(public_path('skins' . DIRECTORY_SEPARATOR . 'default.png'));
        }

        try {
            $storage = Storage::disk('cloaks');
            $image_cloak = Image::read($storage->get($filename));
        } catch (FileNotFoundException $e) {
            $image_cloak = false;
        }
