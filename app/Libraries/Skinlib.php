...
class Skinlib
{
    ...
    public static function createHeadImage(ImageInterface|string $image, $size = 151)
    {
        ...
        $sourceImage = is_string($image) ? Image::read($image) : $image;
        $im = imagecreatefromstring((string) $sourceImage->encodeByMediaType('image/png'));

        imagecopyresized($av, $im, 0, 0, 8 * $mp, 8 * $mp, $size, $size, 8 * $mp, 8 * $mp);
        imagecopyresized($av, $im, 0, 0, 40 * $mp, 8 * $mp, $size, $size, 8 * $mp, 8 * $mp);
        imagedestroy($im);

        return Image::read($av);
    }
    ...
    public static function createPreviewImage(ImageInterface|string $skin_image, $cloak_image, $side = false, $size = 224)
    {
        ...
        $skinSource = is_string($skin_image) ? Image::read($skin_image) : $skin_image;
        $skin = @imagecreatefromstring((string) $skinSource->encodeByMediaType('image/png'));
        if (!$skin){
        ...
        if ($cloak_image && $info = self::isValidCloakImage($cloak_image)) {
            $mp_cloak = $info['scale'];
            $cloakSource = is_string($cloak_image) ? Image::read($cloak_image) : $cloak_image;
            $cloak = @imagecreatefromstring((string) $cloakSource->encodeByMediaType('image/png'));
    ...
    public static function isValidSkinImage(ImageInterface|string $image)
    {
        $imageSize = [
            (is_string($image) ? Image::read($image) : $image)->width(),
            (is_string($image) ? Image::read($image) : $image)->height()
        ];
    ...
    public static function isValidCloakImage(ImageInterface|string $image)
    {
        $imageSize = [
            (is_string($image) ? Image::read($image) : $image)->width(),
            (is_string($image) ? Image::read($image) : $image)->height()
